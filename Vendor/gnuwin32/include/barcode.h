...
# ifdef BUILD_BARCODE_DLL
#  define BARCODE_DLL_IMPEXP	__DLL_EXPORT__
# elif defined(BARCODE_STATIC)
#  define BARCODE_DLL_IMPEXP	 
# elif defined (USE_BARCODE_DLL)
#  define BARCODE_DLL_IMPEXP	__DLL_IMPORT__
# elif defined (USE_BARCODE_STATIC)
#  define BARCODE_DLL_IMPEXP 	 
# else /* assume USE_BARCODE_DLL */
#  define BARCODE_DLL_IMPEXP	__DLL_IMPORT__
# endif
#else /* __WIN32__ */
# define BARCODE_DLL_IMPEXP	 
 ...
 */
BARCODE_DLL_IMPEXP struct Barcode_Item *Barcode_Create(char *text);
BARCODE_DLL_IMPEXP int                  Barcode_Delete(struct Barcode_Item *bc);

/*
 * Encode and print
 */
BARCODE_DLL_IMPEXP int Barcode_Encode(struct Barcode_Item *bc, int flags);
BARCODE_DLL_IMPEXP int Barcode_Print(struct Barcode_Item *bc, FILE *f, int flags);

/*
 * Choose the position
 */
BARCODE_DLL_IMPEXP int Barcode_Position(struct Barcode_Item *bc, int wid, int hei,
			    int xoff, int yoff, double scalef);

/*
 * Do it all in one step
 */
BARCODE_DLL_IMPEXP int Barcode_Encode_and_Print(char *text, FILE *f, int wid, int hei,
 ...
 */
BARCODE_DLL_IMPEXP int Barcode_Version(char *versionname);
